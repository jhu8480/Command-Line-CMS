...
SELECT DISTINCT CONCAT(m.first_name, ' ', m.last_name) AS manager_name, m.employee_id, roles.role_id, department.department_name FROM employee e JOIN employee m ON e.manager_id = m.employee_id JOIN roles ON m.role_id = roles.role_id JOIN department ON roles.department_id = department.department_id;